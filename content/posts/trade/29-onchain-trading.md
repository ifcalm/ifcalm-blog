...
⭐ 所以"哪个更公平"这个问题问错了。准确的说法是**两边各自泄露了不同的东西**：
...
> ⚠️ **本节和第四节含具体平台费率与 2026 年的事件数据，这类内容会过期。** 数字核实于 2026 年 8 月；超过三个月，请按 [Solana 实操分支]({{< ref "/posts/trade/sol" >}}) 的读法，把它们当成需要重新核实的起点，而不是结论。**机制部分不会过期。**
...
按[第 26 篇](/posts/trade/26-screening/)那张表，这直接改变可交易比值——它把某些原本不可行的周期挪进了可行区间。但同时它引入了链上永续特有的风险（合约、清算机制、自有链的验证者集合），这些必须单独尽调。
...
代价在[第 24 篇](/posts/trade/24-onchain-execution/)说过：**你把交易交给了一个特定的中继方**。它解决了「公开可见」，引入了一个新的信任对象。
