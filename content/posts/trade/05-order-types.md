...
第二天开盘，价格直接跳到 80。你的止损在 **80 附近**成交了。

你可能会以为止损单坏了、或者券商没执行。**都不是。它完全按设计工作了**——只是它的设计和你以为的不一样。

订单类型是交易里少数**完全确定、可以精确学会**的东西：没有概率，没有预测，只有规则。也正因如此，因为不了解订单行为而造成的亏损，是最不值得的一类。
...
请注意这句话说了什么、没说什么。它说的是**什么时候开始卖**，它完全没提**卖多少钱**。保安接到指令后冲到市场上，能卖多少是多少——如果那一刻市场上最好的买价是 80，他就卖在 80。

他没有失职。**你给的是一个时机指令，不是一个价格指令。**

⚠️ 所以：**止损单不保证你在止损价出场。** 它只保证在触及止损价时开始尝试卖出，而实际成交价取决于当时的订单簿。

⚠️ 这一条是这一篇里最该记住的，因为它错得最贵。绝大多数人第一次听到"止损"这个词时，脑子里装进去的是"我最多亏到这里"——而这个理解在**平静的市场里恰好总是对的**，于是它被反复验证、变得牢固。它只在跳空、停牌复牌、闪崩这些时候失效，**而那恰恰是你最需要它的时候**。
...
| ⚠️ 延长时段 | **多数美股券商的止损单只在常规时段生效。** 跳空发生在盘前，止损要到 09:30 才变成市价单——盘前那段跌幅它完全管不到 |
...
| 止损单生效时段 | **多数券商仅常规时段**（盘前盘后不触发） | 7×24 |
...
- **止损单不保证止损价。** 跳空、停牌、闪崩时，它会以差得多的价格成交；而且在美股，**多数券商的止损单在盘前盘后不生效**。这不是缺陷，是它的定义。
