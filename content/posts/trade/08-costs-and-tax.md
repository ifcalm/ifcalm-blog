...
成本是交易里**唯一确定为负**的一项。收益是不确定的，成本不是——它每一笔都发生，而且随交易频率线性累积。第一篇讲过，扣除成本后主动交易是负和的；这一篇把「成本」这个词拆开，看看它具体由什么组成。
...
（表里是按线性相加算的，它是上界。按复利精确算，250 次是 **−39.4%**——[第 46 篇]({{< ref "46-behavioral-edge.md" >}})第四节用的是这个口径。低频时两种算法几乎一样，高频时差 10 个百分点，但结论不变：一年被磨掉三四成本金。）

### 打个比方

把账户想成一个**盛水的桶**：收益是往里倒水，成本是桶底的一个洞。

这个洞的大小不由你倒了多少水决定，**由你倒了多少次决定**。单次 0.2% 听起来小到可以忽略——它确实小。可这个洞每进出一次就漏一次，一年 250 次之后，漏掉的是半桶。

⚠️ 这里最反直觉的是**它和你的判断质量完全无关**。上面那半桶水，是在你胜率五五开、判断不好不坏的情况下漏掉的。你可以完全不犯错，照样把账户交易到只剩一半。

⭐ 所以这张表真正的用法不是"吓唬你别做短线"，而是给出一条**门槛**：**在你考虑任何策略之前，这个数字必须先被覆盖掉。** 一个年化 8% 的策略配上每日换手，是数学上的死局——不是它不好，是它还没上场就已经欠了 50%。
...
**机制**：各国有制裁名单（美国是 OFAC 的 SDN 名单），名单上会包含**具体的链上地址**。与被制裁地址交互，可能带来法律和实际后果。
...
1. 合规状态**不是静态的**，今天合规的操作明天可能不是，反之亦然
2. **不要把「现在没事」当作长期结论**——[第二十篇](/posts/trade/20-performance-and-retirement/)的「机制失效」信号里，规则变化是最强的一类
...
- 成本随换手率累积：单次 0.2% 成本 × 每日交易 ≈ **年化 40%–50%**（复利 39%，线性相加 50%）。
