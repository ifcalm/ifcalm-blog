...
在一只流动性正常的美股上打一张市价单，最坏的结果是滑一点点价。在一个薄的币对上打同样一张单，你可能成交在离盘口很远的地方——而且**没有熔断把它停下来，没有涨跌幅限制兜着，也没有人会在事后把这笔交易撤销**。
...
股票市场像一条**修好的高速公路**：有护栏、有限速、有匝道、出了事有救护车。你还是可能出事，但整条路是被设计来兜住失误的。

加密市场是同一辆车、同一个引擎（第二篇那套订单簿），开在**一条没有护栏的山路**上。而且这条路允许你把油门踩得更深（杠杆倍数高得多）。
...
| | U 本位（线性） | 币本位（反向） |
|---|---|---|
| 保证金与盈亏 | 以 USDT / USDC 计 | **以币本身计** |
| 做多时 | 盈亏线性 | ⚠️ 价格下跌 ⇒ 仓位亏损，**同时保证金本身也在贬值**——两头受伤，同样杠杆下强平价比 U 本位更近 |
| 做空时 | 盈亏线性 | 价格下跌 ⇒ 仓位盈利，但赚到的币在贬值——**天然自带一部分对冲** |
| 适合谁 | 想让盈亏可以直接算的人 | 长期持币、想用币做保证金的人 |

币本位的盈亏对价格是**非线性**的（第十篇做基差时会再碰到它）。新手最常见的误判是把币本位做多当成「U 本位做多再多买点币」——它不是，它的下行比 U 本位更陡。

### ⚠️ 用币做保证金 = 双重敞口

即使你交易的是 U 本位合约，不少平台也允许用 BTC、ETH 等作为保证金，按一个**折算率**（haircut，例如 95%）计入可用保证金。

```
用 1 BTC 做保证金，开 BTC 多单
价格跌 10%
  → 合约仓位亏损
  → 保证金那 1 BTC 本身也少了 10% 的价值
  → 保证金率的分子分母同时恶化
```

**你以为的一份敞口，实际上是两份。** 这和第六篇「相关性在危机中趋向 1」是同一件事。折算率还可能在极端行情中被平台临时下调——这是一条要去读平台文档的规则。

### 标记价格（Mark Price）

强平**不是**按最新成交价触发的，而是按**标记价格**——以多家现货交易所合成的**指数价格**为基础，再叠加合约相对指数的基差平滑值（各平台公式不同，常见形式是「指数价 + 基差的移动平均」）。所以标记价格既不是本所的最新价，也不完全等于指数价。
...
| 最大杠杆（零售） | 隔夜约 2 倍（Reg T）；PDT 账户日内 4 倍 | 常见 10–100 倍甚至更高 |
