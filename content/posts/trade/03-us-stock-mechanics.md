...
另外，每年有几个**半日市**（通常是感恩节次日与平安夜），常规时段在 **13:00 ET** 提前收盘。这类日子成交清淡、价差偏宽，属于第十六篇那类「日历上能提前查到」的流动性事件——下单前看一眼交易所日历。
...
长期以来，$1 以上的股票最小报价单位是 **$0.01**。SEC 在 2024 年通过的 Reg NMS 修订引入了**半美分（$0.005）报价档**，适用于价差长期极窄的部分股票，并下调了交易所的接入费上限。⚠️ 这项修订在通过后曾遭到诉讼挑战，各部分的生效时间并不同步，**实际实施进度请以 SEC 的最新公告为准**。若已生效，实际影响是这些股票的价差可以进一步收窄。

### 券商也可能限制你买入

链条里的清算所（NSCC）会向券商收取**清算保证金**，金额随会员客户持仓的波动率和集中度上升。极端行情下这笔要求可以在一夜之间放大数倍。

2021 年 1 月的 GameStop 事件就是这样传导到散户的：多家零售券商因为清算保证金要求骤增，把若干只股票设为**只能平仓、不能开新仓**（position close only），持续了数个交易日。

对你的含义有两条：

- **「能不能买」不只取决于市场，也取决于券商的资本状况和清算所的要求。** 这不是券商「站在对手一边」，是清算链条的机械后果。
- 这种限制最可能出现在最拥挤、波动最大的标的上——而那恰恰是你最想动的时候。
...
- **日内购买力**：被标记为 PDT 且权益达标的账户，日内可用购买力通常是维持保证金超额部分的 **4 倍**；隔夜仍受 Reg T 的 2 倍约束。第四篇对照表里的「零售约 2 倍」指的是隔夜
...
- ⚠️ FINRA 在 2025 年提出以**日内保证金要求**取代 $25,000 门槛的改革提案。这条规则可能在你读到时已经改变，**以 FINRA 与券商的最新说明为准**。
