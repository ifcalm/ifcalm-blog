...
tags: ["计算机网络"]
draft: false
summary: "覆盖第 1–9 讲的 24 道计算与分析题：接入网共享、统计复用的概率计算、四种时延、带宽时延积、HTTP 的 RTT 分析、缓存命中率、DNS 层次、P2P 分发时间与 VoIP 播放缓冲。附完整解答。"
showToc: true
tocOpen: false
---
...
**代价**：每一个块都要为了迁就那唯一一个倒霉的块而**统一多等 69 ms**。这 69 ms 还要叠加采集、编码、解码、播放的开销，很容易把单向时延推过 150 ms 的舒适线。⭐ **这就是固定播放时延的根本矛盾：一个离群值绑架了所有人**——所以实际系统改用自适应播放时延，并且接受丢掉少数离群块（比如取 `q = 58` 就只丢第 4 块）。
...
| 对策 | 做法 | ⭐ 时延代价 |
|---|---|---|
| **FEC（前向纠错）** | 每 n 个块附加一个冗余块，丢任意一个都能恢复 | 必须**等齐这一组**才能解码，⭐ 增加约一个块组的时延；另有 `1/n` 的带宽开销 |
