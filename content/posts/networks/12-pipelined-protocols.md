...
> 📖 **课前阅读**：KR 第 8 版 §3.4.2–§3.4.4
> ⏱ **课时**：75 分钟 · 🧪 **配套**：**Lab 3 直接实现本讲内容** · 📝 PS 2 第 9–13 题
...
1. **计算**窗口大小为 N 的流水线协议的利用率。
2. **追踪** GBN 协议在丢包场景下的完整报文序列。
3. **追踪** SR 协议在同一场景下的报文序列，并指出两者的差异。
4. **证明** GBN 需要 N+1 个序号、SR 需要 2N 个序号。
5. **为一个给定的网络环境选择** GBN 或 SR，并说明理由。
...
*上一篇：[第 11 讲：可靠数据传输原理]({{< ref "11-reliable-data-transfer.md" >}})*
*下一篇：[第 13 讲：TCP——报文段结构、RTT 估计与可靠传输]({{< ref "13-tcp-basics.md" >}})*
