...
> 📖 **课前阅读**：KR 第 8 版 §2.2.5、§2.6.3；RFC 9113（HTTP/2）、RFC 9114（HTTP/3）导读
> ⏱ **课时**：75 分钟 · 🧪 **配套**：Lab 1 Part A · 📝 PS 1 第 16–18 题
...
1. **说明** Web 缓存同时带来的三重收益，并**计算**命中率变化对接入链路流量强度的影响。
2. **解释** `Cache-Control` 主要指令的语义。
3. **对比** CDN 的「深入部署」与「邀请做客」两种哲学。
4. **描述** CDN 如何借助 DNS 完成节点选择。
5. **诊断** HTTP/1.1 的队头阻塞问题，并说明 HTTP/2 解决了哪一层、没解决哪一层。
6. **论证** HTTP/3 为什么必须放弃 TCP 而建在 UDP 上。
...
*上一篇：[第 5 讲：应用层原理与 HTTP]({{< ref "05-application-layer-http.md" >}})*
*下一篇：[第 7 讲：DNS——互联网的目录服务]({{< ref "07-dns.md" >}})*
