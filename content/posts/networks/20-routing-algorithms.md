...
> 📖 **课前阅读**：KR 第 8 版 §5.1–§5.2
> ⏱ **课时**：75 分钟 · 🧪 **配套**：**Lab 4 直接实现本讲的 DV 算法** · 📝 PS 3 第 15–19 题
...
1. **把网络抽象**为带权图，并**按三个维度分类**路由算法。
2. **手工执行** Dijkstra 算法，给出完整的迭代表和最终转发表。
3. **解释**链路状态算法的**震荡**问题及其缓解方法。
4. **手工执行**距离向量的迭代过程直到收敛。
5. **演示**无穷计数问题，并**说明**毒性逆转为什么不能彻底解决它。
6. **对比** LS 与 DV 在报文复杂度、收敛速度与健壮性上的差异。
...
*上一篇：[第 19 讲：IPv6、隧道与泛化转发]({{< ref "19-ipv6-and-generalized-forwarding.md" >}})*
*下一篇：[第 21 讲：域内路由 OSPF 与域间路由 BGP]({{< ref "21-ospf-bgp.md" >}})*
