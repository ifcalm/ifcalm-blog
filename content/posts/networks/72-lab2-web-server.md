...
1. 用 socket API 实现一个 TCP 服务器
2. ⭐ 正确处理 **TCP 是字节流、没有报文边界**这一事实
3. 解析并生成符合 RFC 9110/9112 的 HTTP 报文
4. 实现持续连接与并发处理
5. 实现条件 GET 与缓存语义
...
| 项目 | 分值 | 关键检查点 |
|---|---|---|
| Part 1：基础服务器 | 30 | 6 种状态码正确；⭐ **路径穿越防护（5 分，不通过则本 Part 上限 20**） |
| Part 2：持续连接 | 20 | ⭐ 一条连接上连发两个请求必须都正确响应 |
| Part 3：并发 | 25 | ⭐ 慢请求不阻塞其他请求 |
| Part 4：缓存 | 15 | ⭐ 304 不带实体体 |
| Part 5：客户端 | 10 | 百分位统计正确 |
| **代码质量** | ±5 | 结构清晰、有错误处理、无裸 `except:` |
| **报告** | 含在各 Part | 第 1、2、3 问答不出等于该 Part 扣一半 |
| **合计** | **100** | |

### 自动测试

助教会用一套自动化测试脚本运行你的服务器，包括：
...
⭐ 路径穿越攻击（多种编码变体）
⭐ 一条连接上的流水线请求
⭐ 分片发送的请求（每次只发 1 字节，测试你的循环读）
⭐ 超大首部（测试你的长度限制）
⭐ 并发 100 连接
⭐ 条件 GET
```

📌 **强烈建议自己先跑一遍这些场景。**尤其是「每次只发 1 字节」——这是最能暴露报文边界处理错误的测试。
...
*相关讲次：[第 5 讲（HTTP）]({{< ref "05-application-layer-http.md" >}}) · [第 6 讲（缓存）]({{< ref "06-web-caching-cdn-http2-http3.md" >}}) · [第 9 讲（Socket）]({{< ref "09-socket-programming.md" >}})*
*下一个实验：[Lab 3：可靠传输协议]({{< ref "73-lab3-reliable-transport.md" >}})*
