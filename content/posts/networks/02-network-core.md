...
> 📖 **课前阅读**：KR 第 8 版 §1.3
> ⏱ **课时**：75 分钟 · 📝 **配套**：Problem Set 1 第 3–5 题
...
1. **描述**存储转发（store-and-forward）机制，并**计算** N 跳路径上的存储转发时延。
2. **解释**排队时延与丢包的**成因**，说明它们为什么是分组交换的固有属性而非缺陷。
3. **对比** FDM 与 TDM 两种电路交换的复用方式。
4. **用概率论证明**分组交换在突发流量下的容量优势（统计复用）。
5. **区分**转发（forwarding）与路由（routing）——这是全课程最容易混淆的一对概念。
...
*上一篇：[第 1 讲：互联网是什么]({{< ref "01-what-is-the-internet.md" >}})*
*下一篇：[第 3 讲：时延、丢包与吞吐量]({{< ref "03-delay-loss-throughput.md" >}})*
