...
> 📖 **课前阅读**：KR 第 8 版 §1.5–§1.7
> ⏱ **课时**：75 分钟 · 🧪 **配套**：Lab 1 发布（Wireshark 抓包）
...
1. **列出**互联网五层模型的每一层，说明**它解决什么问题**、有哪些代表协议。
2. **对应** OSI 七层模型与五层模型，并说明多出来的两层去了哪里。
3. **追踪**一份数据从应用层到物理层的**封装**过程，指出每层加了什么头部。
4. **陈述**端到端原则，并用它论证「为什么可靠传输不放在 IP 层」。
5. **解释**沙漏模型，说明 IP 层为什么必须「窄」。
6. **分类**五种基本网络攻击，并指出各自对应的协议层弱点。
...
*上一篇：[第 3 讲：时延、丢包与吞吐量]({{< ref "03-delay-loss-throughput.md" >}})*
*下一篇：[第 5 讲：应用层原理与 HTTP]({{< ref "05-application-layer-http.md" >}})*
