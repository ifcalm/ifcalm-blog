...
> 📖 **课前阅读**：KR 第 8 版 §2.7
> ⏱ **课时**：75 分钟 · 🧪 **配套**：**Lab 2 发布**（并发 HTTP 服务器）
...
1. **写出**可运行的 UDP 客户端与服务器。
2. **写出**可运行的 TCP 客户端与服务器，并**解释**欢迎套接字与连接套接字的区别。
3. **说明**为什么 TCP 应用必须自己做**报文定界**（framing）。
4. **比较**三种并发模型：多线程、I/O 多路复用、异步。
5. **诊断**五类常见的 socket 编程错误。
...
*上一篇：[第 8 讲：电子邮件、P2P 与流媒体]({{< ref "08-email-p2p-streaming.md" >}})*
*下一篇：[第 10 讲：传输层服务、多路复用与 UDP]({{< ref "10-transport-layer-udp.md" >}})*
