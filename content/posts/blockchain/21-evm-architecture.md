...
每个节点都必须执行它、并得出相同结果——**于是每个节点都卡在那里，全网停摆。**

更糟的是，你没法提前检查一段代码会不会停下来（停机问题）。所以这个防御必须在**运行时**做，而且必须让每个节点都得出**完全一致**的"停在哪一步"。
...
前面几个单元在做的事，像**让一屋子人对一份账单的总额达成一致**——加减法，谁算都一样。

这一讲开始，问题变成**让一屋子人对"运行一个程序会得到什么结果"达成一致**。

难点在于：**程序可能永远不停。** 而共识要求每个人都执行完、并给出相同结果——一个死循环就能让全网一起卡死。你甚至没法提前检查它会不会停（这就是停机问题）。

⚠️ 所以下面 **"准"（quasi）这个词是精确的**，不是修辞上的谦虚：
...
⭐ 回到那屋子人：**要让所有人算出一模一样的结果，"数有多大"这件事就不能留任何余地。** 定死 256 位，是把"不同机器上整数宽度不同"这个变量彻底消掉。
