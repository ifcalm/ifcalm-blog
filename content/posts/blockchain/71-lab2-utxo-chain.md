...
**你**会写一个交易验证函数，然后发现自己漏了一个 `if`——**于是可以凭空造币。**

### 打个比方

一条链的"共识规则"听起来很宏大，实际上它就是**验证函数里的那几个 `if`**：一份检查清单，逐条核对，全过才收。

而这份清单的性质和普通业务代码不同：**漏掉一条不会报错，也不会让测试变红。** 它只是让某一类交易悄悄地通过——直到有人发现它。

⚠️ **这个实验最难的地方，是它不会告诉你哪里错了。**

漏掉一个检查，你写的测试**照样全绿**——因为你的测试用的是合法交易，而漏掉的那个 `if` 只在非法交易上才起作用。想发现它，你必须**主动去写攻击**。这个"要自己去证伪自己"的动作，比写实现本身难得多，也是这个实验真正要训练的东西。
...
⭐ **这就是那份检查清单的反面。** 前五个任务在写 `if`，这一个任务在找你漏掉的那个——每复现一次攻击，你就知道了某一条规则在挡什么。
