...
第 19 讲的 BFT 能给你"一旦确定就永不回滚"，但它只撑得住几百个参与者。

第 16 讲的中本聪共识能容纳几百万人，但它永远只给你"越来越可能不会回滚"。
...
前面的人自由地往前探路——走错了可以退回来（这是中本聪那一层，快、开放、可回滚）。而每隔一段，队伍就在身后**打一根桩，钉死**：桩以前的路，谁都不许再改（这是 BFT 那一层，慢、确定、不可逆）。

这一讲讲的就是这两层怎么拼在一起，以及桩打不下去的时候会发生什么。

⚠️ 提前说明这一讲为什么读起来比前几讲累：**它不是一个干净的设计，是两套机制的拼接**。很多细节（epoch、检查点、委员会轮换）单看都有点随意，只有放回"要同时满足上面那两个矛盾目标"这个背景里才讲得通。**遇到想不明白的参数，先回头看它在维护哪一层的性质。**
...
⭐ 这一节讲的是那些桩到底钉得多死：**不是"改不了"，而是"改了一定能查出是谁改的，并且没收他的押金"。** 最终性在这里是一个经济承诺，不是物理约束。
