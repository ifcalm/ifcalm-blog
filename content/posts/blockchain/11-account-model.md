...
合约像一台**自动售货机**：它的逻辑完全确定，投币就出货，从不打瞌睡。

但它**不会自己启动**。它没有闹钟，也没有员工。**必须有人走过来按下按钮**——而且这个人还得自己出电费（Gas）。
...
**历史上的坑**：CREATE2 加上 `SELFDESTRUCT`，曾经允许**在同一个地址上先后部署完全不同的代码**：
...
- **账户状态四个字段**：nonce、balance、storageRoot、codeHash。用 `code.length == 0` 判断 EOA **挡不住构造函数期间的合约**。
...
- **Solana 把"显式读写集"移植到账户模型上**换取并行；代价是开发者必须提前声明所有会触碰的账户，动态调用下很难精确。
