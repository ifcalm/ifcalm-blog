...
⚠️ 代价是**延迟**：一笔交易必须等到下一个块才能被确认。比特币平均 10 分钟，以太坊 12 秒。这是一个刻意的取舍，第 16 讲会讲为什么比特币选了这么长的间隔。
...
>
> 密码学只保证"篡改会被发现"。真正阻止篡改的是"重做后续全部工作的成本高到不划算"——这是一个**经济命题**（第 1 讲、第 17 讲）。
