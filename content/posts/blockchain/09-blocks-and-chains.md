...
| `prev_block_hash` | 链的连接。创世块此处全 0 |
| `merkle_root` | 本块交易的承诺。**交易本身不在头里**，所以头的大小与交易数无关 |
| `timestamp` | 见第五节，不可靠 |
| `bits` | 难度目标的紧凑编码（尾数 + 指数）。第 14 讲展开 |
| `nonce` | 只有 4 字节 = 40 亿种可能。**现代矿机几秒就穷举完**，所以还需要改动 coinbase 交易里的 extraNonce 来扩大搜索空间 |
...
| **硬分叉** | 规则**放宽或改变**：新块老节点不认 | 会永久分裂成两条链，除非全网升级 |
...
- `nonce` 只有 4 字节，现代矿机几秒穷举完，所以实际还要改 coinbase 里的 extraNonce 扩大搜索空间。
