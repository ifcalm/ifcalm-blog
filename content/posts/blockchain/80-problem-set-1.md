...
**(c)** 前提是**只有一个写入者**。⚠️ 去掉它之后，"检查余额"和"扣款"之间就不再原子——两个写入者可以各自读到同一个余额 30，各自判断"够用"，然后各自扣款。⭐ **这正是数据库里的经典竞态，而分布式环境下它无法靠加锁解决，因为没有一个共同的锁。**
...
**(b)** ⭐ 异步的定义是"消息最终会到，但没有时间上界"。⚠️ 既然没有上界，**任何超时都不能证明对方死了**——它可能只是慢。而崩溃和缓慢在观测上完全相同：**都是"没收到消息"。**
...
⚠️ 两个前提各自都足以让 FLP 不适用。⭐ **它不是解决了经典共识问题，而是换了一个问题。**
...
**(c)** ⭐ 因为区块链运行在**部分同步**模型下，不是同步模型。签名解决的是"中转者篡改他人消息"，⚠️ 但它无法解决"网络延迟导致无法区分故障"。⭐ **在部分同步下，即使有签名，`n ≥ 3f+1` 依然是必要的。**
...
**(b)** ⭐ 因为找碰撞时**攻击者可以自由选择两个输入**，而生日悖论让这个自由度把成本从 `2ⁿ` 降到 `2^(n/2)`。⚠️ **哈希的安全级别必须按最弱的那条性质来标称。**
...
⭐ 隐藏性依赖的假设：**哈希的抗原像性 + `r` 有足够熵**。⚠️ 注意这两条缺一不可——如果 `r` 用 `block.timestamp` 生成，熵仍然很低，攻击照样成立。
...
⭐ **加大保证金不能根治**，因为这是一个**比较**关系：只要**操纵收益 > 保证金**，放弃保证金就仍然划算。⚠️ 而操纵收益取决于奖池大小，是可以任意大的。⭐ **真正的解法是 VRF 或 VDF——用密码学消除"选择权"本身。**
...
出块者：
   ...
   ⚠️ 代价只是一个区块奖励——只要稀有 NFT 价值更高，攻击就划算。

普通用户：
   ⭐ 他能【预先模拟】：在一笔交易里先读 blockhash，
      算出结果，若不利就 revert 整笔交易。
   ⟹ ⚠️ 由于 EVM 的原子性，这等于免费重试无限次
      （只损失 Gas）
```
