...
**你**会先写出一棵能跑的 Merkle 树，然后亲手用它伪造一份包含证明——**证明一笔根本不存在的交易在区块里。**

### 打个比方

这像**先按图纸装好一把锁，再自己拿铁丝把它捅开**。

读到"叶子要加 0x00 前缀"这条规则时，它只是一行需要背的注意事项。而当你**亲眼看到不加前缀时伪造证明是怎么成立的**，这条规则就不再需要背了——你知道它挡住了什么。

⚠️ **提前说一句你大概率会卡住的地方**：不是哈希，是**奇数个节点怎么处理**，以及 **n=1 这个退化情况**。这两处的边界条件占了这个实验一半以上的调试时间——而它们不是"细节"，历史上真实的漏洞就出在这里。自检里那句"一定要覆盖 1 和各种奇数"不是客套话。
