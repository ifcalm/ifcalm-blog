...
签名像**火漆印**：印章（私钥）只有你有，印出来的花纹（签名）谁都能对着公开的样本核对。

但数字签名比火漆印多一条关键性质，而这一条恰恰是最容易被忽略的：**火漆印按在哪封信上都一样，数字签名却是"这把私钥"和"这条消息"共同算出来的**——换一条消息，签名就完全不同，也就没法把它从一封信剪下来贴到另一封上。

⚠️ 后面所有的签名漏洞，几乎都是这条性质在某个环节上被打破了：要么消息没被完整地绑进去（重放、跨链重放），要么签名可以在不知道私钥的情况下被改写（可延展性），要么随机数泄露让私钥被反推出来。
...
⚠️ 这一节是火漆印那个比方失效得最彻底的地方：**印章本身没被偷，可对方从两份盖过的文件里，把印章的形状反推了出来。** 而这一切只需要你在两次盖印时用了同一个随机数。
