...
① ⭐ 跨分片调用破坏了 DeFi 依赖的【同步原子性】
   一次跨分片调用需要异步通信和多轮确认，
   ⚠️ 而"在一笔交易里同时操作两个协议"是 DeFi 组合性的基础
...
⭐⭐ **这是一个反复出现的模式：一个安全机制带来的不便，被市场用"引入新的信任"消除了。用户体验改善了，⚠️ 而大多数用户并不知道自己的安全模型已经变了。**
...
⭐ **实际含义：连接一条低安全性的链，不是"给用户多一个选择"，而是把自己的安全性也拉到那条链的水平——至少在跨过去的那部分资产上。** ⚠️ **而这个代价由 A 链的用户承担，他们甚至可能从未用过 B。**
...
⭐ **危害：在一个把公钥当作身份的系统里，攻击者可以宣称拥有一个他实际不控制的公钥。** ⚠️ 这正是"弱 Fiat-Shamir"类漏洞（业内称 Frozen Heart）。
...
⭐⭐ **上面 (b) 的攻击【会通过形式化验证】**——因为代码确实按"份额 = 金额 × 总份额 / 总资产"这个规范执行了。⚠️ **错的是这个规范本身没有考虑总份额极小的情况。**
