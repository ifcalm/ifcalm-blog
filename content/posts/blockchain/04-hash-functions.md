...
密码学在区块链里用得其实很少——**只有四样：哈希、Merkle 树、签名、承诺**。这个单元从零讲起，不假设任何背景。
...
- **确定性**：同样的输入永远给出同样的输出
- **高效**：计算很快（这既是优点也是缺点——第 14 讲会看到 PoW 反过来利用了这一点）
...
⭐ **雪崩效应是"不可预测"的具象表现**：你无法通过微调输入来"接近"某个目标输出。第 14 讲的工作量证明完全建立在这一点上——**除了一个一个试，没有别的办法。**
...
| Merkle 树（第 5 讲） | **抗碰撞** | ⚠️ 攻击者可以构造两棵树，自由选择两边 |
| 承诺方案（第 8 讲） | **抗碰撞 + 隐藏性** | 承诺者自己就是攻击者，两个值都由他选 |
| 地址派生（第 6 讲） | **抗原像** | 从地址反推公钥必须不可行 |
| 工作量证明（第 14 讲） | **不可预测性** | 需要的是"只能穷举" |
...
由于雪崩效应，除了穷举没有捷径。这让"计算"变成了可验证的稀缺资源（第 14 讲）。
...
- ⚠️ **MD5 和 SHA-1 被攻破的是抗碰撞性，抗原像性至今成立。** "MD5 被破解"常被误解。
...
- ⚠️ **以太坊的 `keccak256` ≠ 标准 `SHA3-256`**，填充规则不同，输出完全不同。Go 里必须用 `sha3.NewLegacyKeccak256()`。
