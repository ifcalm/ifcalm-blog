...
slot 2：c (偏移 0, 20 字节)
        d (偏移 20, 8 字节)       ⚠️ 已用 28，e 需要 12，装不下
slot 3：e (偏移 0, 12 字节)
...
例：address owner（几乎不变）+ uint96 count（频繁自增）打包在一起
   ⟹ ⚠️ 每次改 count 都要：SLOAD 整槽 → 掩码 → 移位 → SSTORE
...
用户调用 Proxy 的 setOwner(X)
   ...
   ⟹ ⚠️ Proxy 的 implementation 被改成了 X
...
B 用 delegatecall 调用 C：
   ⭐ msg.sender  = A          （⚠️ 不是 B！保持了 B 收到的值）
   ⭐ address(this) = B
   ⭐ 读写的存储  = B 的存储
...
⭐ **第 ② 条对新用户影响最大**：一个刚拿到 USDC 的人，因为没有 ETH 而**完全无法动用自己的资产**——包括无法把 USDC 换成 ETH。⚠️ **这是一个自锁死的状态。**
