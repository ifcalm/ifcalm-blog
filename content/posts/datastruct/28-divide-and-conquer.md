...
**证明**：考察矩形 `R = [midX − δ, midX + δ] × [p.y, p.y + δ]`——即带内所有纵坐标在 p 之后 δ 之内的点。**注意 R 以中线为对称轴**（不是以 p 为角点），因此中线把它切成的左右两个 δ×δ 正方形，**恰好一个整体位于左半平面、一个整体位于右半平面**。

**每个 δ×δ 正方形内至多 4 个点**：否则把它四等分成 (δ/2)×(δ/2) 的小格，5 个点中必有两点落入同一小格（鸽笼原理），距离 ≤ `(δ/2)·√2 ≈ 0.707δ < δ`。而这两点同属左半或右半平面，与「δ 是各半平面内的最小距离」矛盾。

于是 R 内至多 8 个点（含 p 自己），**p 只需与 y 序排在它之后的至多 7 个点比较**。∎

⚠️ 若 p 的纵坐标之差超过 δ，两点距离已 > δ，无需比较——这正是「只看后面 7 个」而非「看整条带」的依据。
...
```
     ...
     └────────────┼────────────┘  ▼   ← 底边过 p
      整体在左半平面 │ 整体在右半平面
     每个 δ×δ 正方形内至多 4 个点（四等分 + 鸽笼）
     ⟹ 矩形内至多 8 个点 ⟹ p 只需比较 7 个
```
