...
| 单模式、实践最快 | **Boyer-Moore / Two-Way** | 平均**亚线性**（坏字符规则可整段跳过文本）；glibc 的 `strstr` 用 Two-Way |
...
⭐ **一条实用提醒**：**Go 的 `strings.Contains` / `strings.Index` 已经高度优化**，但它用的**不是** KMP 也不是 Boyer-Moore——实际策略是「用首两字节做预筛的暴力扫描 + `bytealg` 的 SIMD 加速；当失配次数超过阈值时，切换到 **Rabin-Karp**」。⚠️ 这说明一件事：**在真实的短文本上，一个常数极小、对 SIMD 友好的"笨"算法，往往打败渐近更优但常数大、且需要预处理的聪明算法。**KMP 的 Θ(n+m) 保证要到模式很长、或文本对抗性很强时才体现价值。

**除非你有特殊需求（多模式、前缀查询、反复查询同一文本），不要自己实现字符串匹配。**
